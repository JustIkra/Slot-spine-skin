...
def _kf(time, angle=None, x=None, y=None, curve=EASE):
    ...
    if angle is not None:
        kf["value"] = round(angle, 2)
    if x is not None:
        kf["x"] = round(x, 2)
    if y is not None:
        kf["y"] = round(y, 2)
    if curve:
        kf["_easing"] = list(curve)
    return kf


def absolute_curve(t0, v0, t1, v1, easing):
    if len(easing) != 4 or not all(math.isfinite(v) for v in [t0, v0, t1, v1, *easing]):
        raise ValueError("Expected four finite easing coordinates")
    if t1 <= t0 or not (0 <= easing[0] <= 1 and 0 <= easing[2] <= 1):
        raise ValueError("Easing requires increasing times and control times within the interval")
    x1, y1, x2, y2 = easing
    return [t0+(t1-t0)*x1, v0+(v1-v0)*y1, t0+(t1-t0)*x2, v0+(v1-v0)*y2]


def finish_animation(animation):
    animation = copy.deepcopy(animation)
    for timelines in animation.get("bones", {}).values():
        for kind, keys in timelines.items():
            fields = ("value",) if kind in ("rotate", "translatex", "translatey", "scalex", "scaley") else ("x", "y")
            default = 1 if kind.startswith("scale") else 0
            for index, key in enumerate(keys):
                easing = key.pop("_easing", None)
                if easing is None or index == len(keys)-1:
                    continue
                following = keys[index+1]
                key["curve"] = [value for field in fields for value in absolute_curve(
                    key.get("time", 0), key.get(field, default),
                    following.get("time", 0), following.get(field, default), easing)]
    return animation
...
def build_spine_json(config):
    ...
    for anim_name in config.get("animations", ["idle"]):
        if anim_name in PRESETS:
            ...
            if data:
                spine["animations"][anim_name] = finish_animation(data)
        else:
